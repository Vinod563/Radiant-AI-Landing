...
const PROGRESSION = {
  ...
  Train: {
    ...
    detail: 'Pick a few low-risk, high-volume decisions and let AI handle them end to end under supervision. Measured pilots are what turn training into trust and build the case for the next investment.',
...
const ROLE_STEP = {
  ...
  biz: {
    phase: 'Adopt',
    title: 'Pick one high-volume workflow to automate end to end',
    detail: 'Choose a single operational workflow you own, and measure what happens when AI handles it end to end. One visible, measured win does more for adoption than a portfolio of stalled pilots.',
    service: 'Adoption & change-management support',
  },
  consultant: {
    phase: 'Train',
    title: 'Build curated learning paths for the teams you advise',
    detail: 'As an AI Practitioner, your leverage is enablement. Curated, role-based learning paths give client teams a repeatable way to move from awareness to applied capability.',
...
export function suggestedNextSteps({ role, stageIndex = 1, sectionAverages = {} } = {}) {
  ...
  return {
    ...
    currentPhase,
    band,
    intro: `Your path through Radiant's delivery model — Assess → Train → Adopt → Scale → Sustain — starting from where your organization is today (${currentPhase}).`,
